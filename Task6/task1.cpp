...
public:
  ...
  public:
    ...
    bool valid_move(int from_x, int from_y, int to_x, int to_y) const override {
      return (abs(from_x - to_x) <= 1 && abs(from_y - to_y) <= 1) && (from_x != to_x || from_y != to_y);
    }
    std::string symbol() const override {
      return color == Color::WHITE ? "W_K" : "B_K";
    }
  };
  ...
  public:
    ...
    bool valid_move(int from_x, int from_y, int to_x, int to_y) const override {
      ...
      return ((dx == 2 && dy == 1) || (dx == 1 && dy == 2)) && (from_x != to_x || from_y != to_y);
    }
    std::string symbol() const override {
      return color == Color::WHITE ? "W_N" : "B_N";
    }
  };
      ...
      for (int y = 7; y >= 0; --y) {
        ...
        for (int x = 0; x < 8; ++x) {
          ...
          if (chessboard.squares[x][y]) {
            cout << chessboard.squares[x][y]->symbol(); // Assuming symbol() returns a string like "W_K"
          } else {
            cout << "   "; // Three spaces to match the length of "W_K"
          }
          cout << ']';
        }
        cout << "|" << endl;
